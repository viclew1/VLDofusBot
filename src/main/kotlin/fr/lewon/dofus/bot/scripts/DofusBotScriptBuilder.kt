...
abstract class DofusBotScriptBuilder(val name: String, val isDev: Boolean = false) {
    ...
    fun buildScript(): DofusBotScript {
        return object : DofusBotScript(this) {
            override fun execute(logItem: LogItem, gameInfo: GameInfo, scriptValues: VldbScriptValues) {
                doExecuteScript(logItem, gameInfo, scriptValues)
            }
        }
    }

    protected abstract fun doExecuteScript(logItem: LogItem, gameInfo: GameInfo, scriptValues: VldbScriptValues)
