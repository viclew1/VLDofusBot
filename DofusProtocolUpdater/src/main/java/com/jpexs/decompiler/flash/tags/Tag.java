...
public abstract class Tag implements NeedsCharacters, Exportable, Serializable {
    ...
    private static final long serialVersionUID = 871092444696115868L;
    ...
    protected byte[] getHeader(int dataLength) {
        ...
        try {
            ...
            SWFOutputStream sos = new SWFOutputStream(baos, this.swf.version);
    ...
    public void writeTag(SWFOutputStream sos) throws IOException {
        if (Configuration._debugCopy.get() || this.isModified()) {
    ...
    public byte[] getData() {
        ...
        if (Configuration._debugCopy.get()) {
            byte[] originalData = this.getOriginalData();
            if (originalData != null) {
                os = new CopyOutputStream(os, new ByteArrayInputStream(this.getOriginalData()));
            }
        }

        try (SWFOutputStream sos = new SWFOutputStream(os, this.getVersion())) {
            this.getData(sos);
    ...
    public final byte[] getOriginalData() {
        ...
        int pos = (int) (this.originalRange.getPos() + this.originalRange.getLength() - dataLength);
    ...
    private boolean isLongOriginal() {
        int shortLength = this.originalRange.getArray()[(int) this.originalRange.getPos()] & 0x003F;
    ...
    public void getNeededCharactersDeep(Set<Integer> needed) {
        ...
        this.getNeededCharacters(needed2);

        while (visited.size() != needed2.size()) {
            for (int characterId : needed2) {
                if (!visited.contains(characterId)) {
                    visited.add(characterId);
                    if (this.swf.getCharacters().containsKey(characterId)) {
                        this.swf.getCharacter(characterId).getNeededCharacters(needed2);
                        break;
                    }
                }
            }
    ...
    public void getTagInfo(TagInfo tagInfo) {
        ...
        this.getNeededCharactersDeep(needed);
        ...
        if (this instanceof CharacterTag) {
            ...
            Set<Integer> dependent = this.swf.getDependentCharacters(characterId);
            if (dependent != null) {
                if (dependent.size() > 0) {
