...
abstract class WorldMapHelper(val name: String, val icon: UiResource) {
    ...
    private fun buildMapDrawCell(map: DofusMap, subAreaMaps: Collection<DofusMap>): MapDrawCell {
        ...
        return MapDrawCell(
            ...
            x = map.posX,
            y = map.posY,
            topLeft = Offset(x * ExplorationUIUtil.CellSize, y * ExplorationUIUtil.CellSize),
            topRight = Offset((x + 1) * ExplorationUIUtil.CellSize, y * ExplorationUIUtil.CellSize),
            bottomLeft = Offset(x * ExplorationUIUtil.CellSize, (y + 1) * ExplorationUIUtil.CellSize),
            bottomRight = Offset((x + 1) * ExplorationUIUtil.CellSize, (y + 1) * ExplorationUIUtil.CellSize),
