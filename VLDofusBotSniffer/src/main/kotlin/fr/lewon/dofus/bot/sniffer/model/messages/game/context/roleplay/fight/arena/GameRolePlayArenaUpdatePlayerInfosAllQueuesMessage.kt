package fr.lewon.dofus.bot.sniffer.model.messages.game.context.roleplay.fight.arena

import fr.lewon.dofus.bot.sniffer.model.types.game.context.roleplay.fight.arena.ArenaRankInfos
import fr.lewon.dofus.bot.core.io.stream.ByteArrayReader
import fr.lewon.dofus.bot.sniffer.model.messages.NetworkMessage
import fr.lewon.dofus.bot.sniffer.model.types.NetworkType
import fr.lewon.dofus.bot.sniffer.model.ProtocolTypeManager
import fr.lewon.dofus.bot.core.io.stream.BooleanByteWrapper

open class GameRolePlayArenaUpdatePlayerInfosAllQueuesMessage : GameRolePlayArenaUpdatePlayerInfosMessage() {
	lateinit var team: ArenaRankInfos
	lateinit var duel: ArenaRankInfos
	override fun deserialize(stream: ByteArrayReader) {
		super.deserialize(stream)
		team = ArenaRankInfos()
		team.deserialize(stream)
		duel = ArenaRankInfos()
		duel.deserialize(stream)
	}
	override fun getNetworkMessageId(): Int = 9681
}
